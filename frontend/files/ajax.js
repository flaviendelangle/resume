/**
 * Set of functions aiming at simplifying the creation and the management of a requestio between the extension and the server.
 *
 * @param  object  values           Data required to create the request and decide the behaviour of the script
                                    Values example :
        {
            method  : 'GET',
            domain  : 'zzzelp', (can be 'zzzelp', 'fourmizzz' or 'zzzelp_interne')
            url     : 'profile',
            data    : <POST DATA>,
            force   : 1, (if this value is 2, the script will ask for a new authentication)

        }
 * @param  callBacks                Functions to execute after receiving the response
                                    There is three different callBacks needed :
                                        -  success                  : executed if the data have been successfully retrieved
                                        - authentication_issue      : the authentication is not valid
                                        - unknown_player            : the user is not registered on the website
**/
